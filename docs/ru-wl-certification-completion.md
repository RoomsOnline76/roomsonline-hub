...
| 2 | Discounts pushed every 24 h | General declarations → `Push_PutLongStayDiscounts_RQ` / `Push_PutLastMinuteDiscounts_RQ` | **Closed** (14 Aug 2026) — `ru-discounts-daily` registered at `0 4 * * *` invoking `cron-refresh-ru-discounts`; `config.toml` and the cert portal `EXPECTED_JOBS` row both declare 04:00 UTC | Daily runs log one `refresh_discounts` row per connected property; the Refresh-compliance panel turns amber past 24 h. |
...
| Discounts every 24 h | fully supported | `ru-discounts-daily` — daily 04:00 UTC, `cron-refresh-ru-discounts` |
