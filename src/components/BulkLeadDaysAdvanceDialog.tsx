...
interface BulkLeadDaysAdvanceDialogProps {
  open: boolean;
...
export function BulkLeadDaysAdvanceDialog({ 
  open, 
  onOpenChange,
  propertyId,
  propertyName,
  ...
  const handleCreateRule = async () => {
    ...
    const leadDays = parseInt(leadDaysAdvance, 10);
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>
            {propertyName ? `Set advance booking requirement for ${propertyName}` : 'Set how far in advance guests must book'}
          </DialogDescription>
        </DialogHeader>
              ...
              effectiveRoomTypes.map((room) => (
                <div key={room.id || room.name} className="flex items-center justify-between p-2 hover:bg-muted rounded">
                  <div className="flex items-center">
                    <Checkbox
                      id={`leadadvance-${room.id || room.name}`}
                    ...
                    />
                    <label htmlFor={`leadadvance-${room.id || room.name}`} className="text-sm cursor-pointer ml-2">
                      {room.name}
                    </label>
                  </div>
                </div>
          ...
          <div className="col-span-8 space-y-4">
            ...
            <div className="border rounded-lg p-6 space-y-4">
              <div className="space-y-2">
                <Label>Advance Lead Days</Label>
                <div className="flex gap-2 items-center">
                  <Input
                    type="number"
                    min="0"
                    value={leadDaysAdvance}
                    onChange={(e) => setLeadDaysAdvance(e.target.value)}
                    className="w-32"
                  />
                  <span className="text-sm text-muted-foreground">days before check-in</span>
                </div>
                <p className="text-xs text-muted-foreground">
                  Guests must book at least this many days before their check-in date
                </p>
              </div>
              ...
              <div className="space-y-2">
                ...
                <div className="flex flex-wrap gap-3">
                  <div className="flex items-center space-x-2">
                    <Checkbox
                      id="leadadvance-allDays"
                      checked={selectedDays.allDays}
                      onCheckedChange={() => toggleDay("allDays")}
                    />
                    <label htmlFor="leadadvance-allDays" className="text-sm cursor-pointer font-medium">All days</label>
                  </div>
                  {["sunday", "monday", "tuesday", "wednesday", "thursday", "friday", "saturday"].map((day) => (
                    <div key={day} className="flex items-center space-x-2">
                      ...
                      />
                      <label htmlFor={`leadadvance-${day}`} className="text-sm cursor-pointer capitalize">{day.slice(0, 3)}</label>
                    </div>
                  ))}
                </div>
              </div>
