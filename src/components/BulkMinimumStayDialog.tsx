...
interface BulkMinimumStayDialogProps {
  open: boolean;
...
export function BulkMinimumStayDialog({ 
  open, 
  onOpenChange,
  propertyId,
  propertyName,
  ...
  const handleCreateRule = async () => {
    ...
    const minStay = parseInt(minimumStay, 10);
    if (isNaN(minStay) || minStay < 0) {
      toast.error("Please enter a valid minimum stay");
      return;
    }
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>
            {propertyName ? `Set minimum stay for ${propertyName}` : 'Set minimum stay requirement for selected rooms'}
          </DialogDescription>
        </DialogHeader>
              ...
              effectiveRoomTypes.map((room) => (
                <div key={room.id || room.name} className="flex items-center justify-between p-2 hover:bg-muted rounded">
                  <div className="flex items-center">
                    ...
                    />
                    <label htmlFor={`minstay-${room.id || room.name}`} className="text-sm cursor-pointer ml-2">
                      {room.name}
                    </label>
                  </div>
                </div>
              ...
              <div className="space-y-2">
                ...
                <div className="flex flex-wrap gap-3">
                  <div className="flex items-center space-x-2">
                    ...
                    />
                    <label htmlFor="minstay-allDays" className="text-sm cursor-pointer font-medium">All days</label>
                  </div>
                  {["sunday", "monday", "tuesday", "wednesday", "thursday", "friday", "saturday"].map((day) => (
                    <div key={day} className="flex items-center space-x-2">
                      ...
                      />
                      <label htmlFor={`minstay-${day}`} className="text-sm cursor-pointer capitalize">{day.slice(0, 3)}</label>
                    </div>
                  ))}
                </div>
              </div>
