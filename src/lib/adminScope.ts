...
/** Rentals United IT tester account. Its scope is data-driven, never hardcoded. */
...
/**
 * Scope ids for an admin, taken verbatim from `scoped_admin_properties`.
 *
 * Deliberately has no hardcoded fallback: a tester account must never be able to
 * fall back onto live trading properties because a lookup came back empty. The
 * tester's membership is maintained in the database (the RentalsUnited TEST
 * portfolio), so changing that portfolio changes what the account can see.
 */
