...
export default function AdminSystemHealth() {
  ...
  const runHealthCheck = useMutation({
    ...
    onSuccess: (data) => {
      toast({
        ...
        description: `Status: ${data.overall_status}. ${data.summary?.healthy || 0} healthy, ${data.summary?.failed || 0} failed.`,
      });
      queryClient.invalidateQueries({ queryKey: ['health-checks'] });
    },
    onError: (error) => {
  ...
  const componentStats = useMemo(() => {
    ...
    components.forEach(comp => {
      ...
      const uptime = checks.length > 0 
        ? ((healthyChecks + degradedChecks) / checks.length) * 100 
        : 0;

      stats[comp.component_key] = {
        lastStatus: lastCheck?.status || 'unknown',
        lastChecked: lastCheck 
          ? formatTimeAgo(new Date(lastCheck.checked_at))
          : 'Never',
        avgLatency,
    ...
    const rows = [
      ...
      ...components.map(comp => {
        ...
        return [
          ...
          stats?.lastStatus || 'unknown',
          stats?.avgLatency || 0,
          (stats?.uptimePercentage || 0).toFixed(2),
          stats?.failureCount || 0,
          stats?.lastChecked || 'Never',
        ];
      }),
    ];
  ...
  return (
    <AppLayout>
      ...
      <p className="text-muted-foreground mb-6 -mt-4">Monitor the health and performance of all system components</p>
      ...
      <div className="flex flex-col sm:flex-row justify-between gap-4 mb-6">
        <div className="flex items-center gap-2 text-sm text-muted-foreground">
          ...
          <span>
            Last checked: {lastCheckTime ? formatTimeAgo(lastCheckTime) : 'Never'}
          </span>
        ...
        <Card>
          ...
          <CardContent>
            {isLoading ? (
              ...
              <HealthStatusBadge 
                status={
                  overallStats.failed > 0 ? 'failed' :
                  overallStats.degraded > 0 ? 'degraded' :
                  'healthy'
                }
                size="lg"
              />
        ...
        <Card>
          ...
          <CardContent>
            {isLoading ? (
              ...
              <p className={cn(
                "text-2xl font-bold",
                overallStats.uptime >= 99 ? "text-green-600 dark:text-green-400" :
                overallStats.uptime >= 95 ? "text-yellow-600 dark:text-yellow-400" :
          ...
          <CardContent>
            {isLoading ? (
              ...
              <div className="flex items-baseline gap-2">
                <p className={cn(
                  "text-2xl font-bold",
                  overallStats.failed > 0 ? "text-red-600 dark:text-red-400" : "text-green-600 dark:text-green-400"
                )}>
                  {overallStats.failed}
                </p>
