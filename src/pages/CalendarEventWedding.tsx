...
const CalendarEventWedding = () => {
  ...
  const fetchProperties = async (adminStatus: boolean, email: string) => {
    try {
      ...
      if (urlProperty && eventProperties.find((p: Property) => p.id === urlProperty)) {
    ...
    <div className="min-h-screen bg-background">
      ...
      <div className="container mx-auto px-4 py-8">
        ...
        <Tabs value="event" className="mb-6">
          <TabsList className="grid w-full max-w-md" style={{ gridTemplateColumns: `repeat(${(hasAccommodation ? 1 : 0) + 1 + (hasConference ? 1 : 0)}, 1fr)` }}>
            {hasAccommodation && (
