...
export default function DevOverview() {
  ...
  const loadSystemStatus = async () => {
    try {
      ...
      // Load PMS credentials to determine adapter status
      ...
      (pmsCredentials || []).forEach((cred: any) => {
        if (!adapterMap.has(cred.system_type)) {
          adapterMap.set(cred.system_type, []);
        }
      ...
      // Simulated edge function status (in production, this would come from monitoring)
      const edgeFunctions = [
                  ...
                  >
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-xs text-muted-foreground">
                          {adapter.propertiesCount} properties
                        </p>
                      </div>
                    </div>
