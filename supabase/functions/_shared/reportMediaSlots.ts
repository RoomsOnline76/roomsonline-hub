...
export interface MediaSlotDefinition {
  ...
  title: string;
  hint: string;
  /** One image per row (full width) or two-up. */
  layout: "full" | "half";
}

export const REPORT_MEDIA_SLOTS: MediaSlotDefinition[] = [
  {
    key: "channel_hits",
...
export const findMediaSlot = (key: string): MediaSlotDefinition | undefined =>
  REPORT_MEDIA_SLOTS.find((slot) => slot.key === key);
