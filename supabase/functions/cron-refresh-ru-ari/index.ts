    ...
    for (const prop of properties) {
      ...
      try {
        ...
        const { data, error } = await supabase.functions.invoke('push-property-to-ru', {
          body: { property_id: prop.id, action: 'refresh_ari', trigger: 'cron_daily_ari', verify_readback: false, verify_discount_readback: false, verify_availability_readback: false },
        });
