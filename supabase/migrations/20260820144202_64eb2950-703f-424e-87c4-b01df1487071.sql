...
BEGIN
  ...
  ELSE
    ...
    ELSE
      ...
      ELSIF NEW.status IS DISTINCT FROM OLD.status
        ...
        _change := 'cancelled';
      ELSIF NEW.status IS DISTINCT FROM OLD.status
         AND lower(COALESCE(NEW.status, '')) IN ('checked_in', 'in_house', 'confirmed', 'guaranteed') THEN
        _change := 'confirmed';
  ...
  IF _recent AND _change NOT IN ('cancelled', 'no_show', 'confirmed') THEN
    RETURN COALESCE(NEW, OLD);
  END IF;

  -- One pending job per booking AND change kind: a status change queued behind a notes change
  -- used to be dropped by the dedupe key, so the channel never heard about it.
